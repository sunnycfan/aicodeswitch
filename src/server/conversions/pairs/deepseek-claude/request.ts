/**
 * DeepSeek Reasoning → Claude Messages request conversion.
 *
 * Self-contained: does not depend on other pair directories.
 * DeepSeek uses the same wire format as OpenAI Chat Completions.
 */

import { completionsToClaudeTools } from '../../utils/tool-schema.js';

/**
 * Convert DeepSeek Reasoning request to Claude Messages request.
 */
export function deepseekToClaude(body: any): any {
  const messages: any[] = [];
  ...
  for (const msg of body.messages) {
    ...
    const content = convertDeepseekMessageToClaude(msg);
    if (content) {
      const role =
        msg.role === 'tool'
          ? 'user'
...
/**
 * Convert a single DeepSeek/Completions message into Claude content.
 */
function convertDeepseekMessageToClaude(msg: any): any {
