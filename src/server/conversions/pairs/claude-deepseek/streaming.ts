/**
 * DeepSeek Reasoning SSE → Claude Messages SSE converter.
 *
 * Self-contained: does not depend on other pair directories.
 * DeepSeek uses the same SSE format as OpenAI Chat Completions, with the
 * addition of reasoning_content (handled as thinking blocks).
 */
...
/**
 * Stateful converter: DeepSeek SSE → Claude Messages SSE.
 */
export class DeepseekToClaudeConverter implements StreamConverter {
  ...
  /** Accumulated tool calls keyed by their index */
